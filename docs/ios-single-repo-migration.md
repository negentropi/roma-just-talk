...
- provider API-key draft verification candidate and save-after-verify policy for iOS provider-key entry; macOS AI-enhancement provider-key entry shares draft enablement and environment-reference resolution while preserving shell key storage
...
- vocabulary, word-replacement, backup dictionary insert, vocabulary backup record shape, word-replacement backup import planning, word-replacement edit planning, and dictionary draft submit rules; platform shells still own macOS SwiftData dictionary storage and persistence errors
...
- macOS and iOS local Whisper model directory creation, local `.bin` listing, download URL/file path lookup, runtime selected-model file resolution, final downloaded-file installation, bootstrap-model availability, Core ML support checks, model deletion, download progress keys/math, percent text, iOS downloaded/download-progress state assembly, download prompt copy, and local-model operation alert copy delegate to `VoiceInkWhisperModelFiles`/`VoiceInkWhisperModelDownloadProgress`/`VoiceInkWhisperModelDownloadState`/`VoiceInkWhisperModelOperationAlertPresentation`; macOS uses `VoiceInkWhisperLocalModelFile` directly while platform shells still own URLSession task state, UI, and runtime model state.
...
- macOS dictionary backup vocabulary records use `VoiceInkVocabularyWordBackup` for the Codable export/import shape and shared trim/duplicate import policy; macOS still adapts to SwiftData `VocabularyWord` and owns persistence errors.
- macOS dictionary backup word replacements use `VoiceInkDictionaryPolicy.wordReplacementBackupImportPlan` for shared duplicate skipping, invalid-row counting, and trimmed import records; macOS still adapts the accepted rules to SwiftData `WordReplacement` and owns persistence errors/cache invalidation.
