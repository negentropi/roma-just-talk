...
- `iOS/VoiceInk-ios/LocalModelManager.swift` validates model-download HTTP responses through `VoiceInkWhisperModelDownloadResponsePolicy`; iOS onboarding and local-model management render downloaded/download-progress state, operation confirmations, and operation failure alerts through `VoiceInkWhisperModelDownloadState`/`VoiceInkWhisperModelDownloadProgress`/`VoiceInkWhisperModelOperationConfirmationPresentation`/`VoiceInkWhisperModelOperationAlertPresentation`, while the iOS shell still owns URLSession task/progress state and file-operation execution.
...
21. iOS recording/retry mode selection and settings mode summaries route presentation through `VoiceInkModeSelectionPresentation` and `VoiceInkModeSummaryPresentation`.
22. iOS local model operation confirmations and failure alerts route through `VoiceInkWhisperModelOperationConfirmationPresentation` and `VoiceInkWhisperModelOperationAlertPresentation`.
