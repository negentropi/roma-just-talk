...
- power-mode stored config Codable shape, legacy key decoding, trigger-to-rule adaptation, app/website/default matching, website URL normalization, save validation, auto-send key stored value/display/enablement state, and active-session snapshot Codable shape with cleanup legacy decoding; macOS still owns OS app/website detection, SwiftUI editing/alerts, active-session application/restoration, shortcuts, auto-send key execution, and config/session persistence timing
