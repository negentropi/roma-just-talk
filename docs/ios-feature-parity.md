...
| M08 | Per-language Whisper prompt, trailing space, VAD, prewarm, preview controls | [`ModelSettingsView`](../VoiceInk/Views/ModelSettingsView.swift) | `partial`: iOS exposes shared live-transcript and batch VAD preferences, but per-language prompt, trailing-space, and prewarm controls remain incomplete | shared/iOS | Persistence and observable runtime effect for each setting. |
...
| ID | Capability | macOS evidence | iOS state and implementation | Class | Verification |
|---|---|---|---|---|---|
| S01 | Resumable onboarding with permission, model, tutorial, skip, and reset | macOS onboarding views and shared presentation/state | `partial`: [`OnboardingView`](../iOS/VoiceInk-ios/OnboardingView.swift) now includes keyboard setup with a Set Up Later path; persisted resume, microphone permission setup, a complete tutorial, and normal reset entry remain missing | shared/iOS | Kill/resume at each step, skip, permission failure, model failure, tutorial, and reset. |
| S02 | Dedicated permission status and recovery screen | [`PermissionsView`](../VoiceInk/Views/PermissionsView.swift) | `partial`: [`KeyboardSetupView`](../iOS/VoiceInk-ios/KeyboardSetupView.swift) is reachable from Settings and re-verifies keyboard activation/Full Access after returning; microphone remains recording-time only | shared/iOS | Keyboard Settings return needs simulator/device proof; microphone readiness still needs a dedicated refreshable status and recovery path. |
