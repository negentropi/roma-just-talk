...
- power-mode stored config Codable shape, legacy key decoding, UserDefaults config and active-config-id persistence, trigger-to-rule adaptation, config-list lookup/mutation, app/website/default matching, website URL normalization, save validation, auto-send key stored value/display/enablement state, active-session snapshot Codable shape with cleanup legacy decoding, and active-session UserDefaults persistence; macOS still owns OS app/website detection, SwiftUI editing/alerts, active-session application/restoration, shortcuts, auto-send key execution, and config/session persistence timing
