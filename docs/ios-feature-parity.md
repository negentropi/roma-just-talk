...
| M08 | Per-language Whisper prompt, trailing space, VAD, prewarm, preview controls | [`ModelSettingsView`](../VoiceInk/Views/ModelSettingsView.swift) | `partial`: iOS now exposes the shared per-model live-transcript preference for streaming-capable cloud modes, but per-language prompt, trailing-space, VAD, and prewarm controls remain incomplete | shared/iOS | Persistence and observable runtime effect for each setting. |
