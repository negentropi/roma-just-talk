...
- power-mode app/website trigger record shape, trigger-to-rule adaptation, app/website/default matching, website URL normalization, save validation, and auto-send key stored value/display/enablement state; macOS still owns OS app/website detection, SwiftUI editing/alerts, active-session application, shortcuts, auto-send key execution, and config persistence
...
- macOS Power Mode app/website trigger arrays use `VoiceInkPowerModeAppConfig` and `VoiceInkPowerModeURLConfig`, so the trigger record Codable shape, id equality, and policy-rule adaptation live in shared core while `PowerModeConfig` storage and OS app picker discovery stay in the macOS shell.
- macOS `PowerModeConfig`, `PowerModeConfigView`, Power Mode cards, and `CursorPaster` use `VoiceInkAutoSendKey` for the stored auto-send option, picker labels, and enablement check; macOS still owns `CGEvent` key posting after paste.
