...
- recording-state vocabulary, rolling-preload preview eligibility, shortcut-action eligibility, recorder UI toggle mapping, recorder-session shortcut activity policy, and recording transcription draft state for pending/canceled rows; platform shells still own recorder windows, timers, audio capture, and OS-specific session state
...
- Whisper and VAD model file metadata, including platform-base Whisper model directory creation, downloaded local-model file records, runtime model-name to downloaded-file resolution, model/sidecar file construction, downloaded-state detection, bootstrap-model availability, Core ML support policy, final `.bin` install/replacement, model/sidecar deletion, and accepted HTTP download status policy
...
- macOS and iOS local Whisper model directory creation, local `.bin` listing, download URL/file path lookup, runtime selected-model file resolution, final downloaded-file installation, bootstrap-model availability, Core ML support checks, model deletion, download progress keys/math, percent text, iOS downloaded/download-progress state assembly, and download prompt copy delegate to `VoiceInkWhisperModelFiles`/`VoiceInkWhisperModelDownloadProgress`/`VoiceInkWhisperModelDownloadState`; macOS uses `VoiceInkWhisperLocalModelFile` directly while platform shells still own URLSession task state, UI, and runtime model state.
...
- `iOS/VoiceInk-ios/LocalModelManager.swift` validates model-download HTTP responses through `VoiceInkWhisperModelDownloadResponsePolicy`; iOS onboarding and local-model management render downloaded/download-progress state and download prompts through `VoiceInkWhisperModelDownloadState`/`VoiceInkWhisperModelDownloadProgress`, while the iOS shell still owns URLSession task/progress state and user-facing download errors.
...
- iOS `RecordingManager` uses `VoiceInkRecordingState` and the shared active-recording predicate directly, currently using the shared `.idle`/`.recording` subset while preserving iOS-specific permission, sheet, App Group, and audio-session behavior in the shell.
