...
- `iOS/VoiceInk-ios/NotesListView.swift` also derives its fastest-model note-list summary from `VoiceInkPerformanceAnalyzer`, so iOS consumes the same shared model performance interface as the macOS metrics panels while keeping the iOS UI shell separate.
