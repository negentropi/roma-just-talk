...
- AI-enhancement error vocabulary, user-facing descriptions, and retry/backoff decision policy; platform shells still own provider transport, provider-specific error mapping, sleep timing, and logging
...
- macOS AI-enhancement failures throw `VoiceInkAIEnhancementError` and route retry decisions through `VoiceInkAIEnhancementRetryState` from `VoiceInkCore`; macOS still owns LLMkit/Ollama/Local CLI error mapping, sleep timing, and logging.
