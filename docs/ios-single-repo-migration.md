...
- power-mode stored config Codable shape, legacy key decoding, trigger-to-rule adaptation, app/website/default matching, website URL normalization, save validation, and auto-send key stored value/display/enablement state; macOS still owns OS app/website detection, SwiftUI editing/alerts, active-session application, shortcuts, auto-send key execution, and config persistence timing
