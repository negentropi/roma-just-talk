...
- macOS recorder state now uses `VoiceInkRecordingState` directly, while `RecorderUIToggleAction` remains a shell alias over `VoiceInkRecorderUIToggleAction`; `RecordingShortcutManager`, `RecorderUIManager`, and rolling preload use shared state gates while macOS owns recorder UI/window execution.
