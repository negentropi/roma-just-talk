...
- macOS `PowerModeManager`, `PowerModePopover`, and `PowerModeConfigView` call `VoiceInkPowerModePolicy`, `VoiceInkPowerModeConfigurationPreference`, and shared `PowerModeConfig` array helpers directly for config/active-id storage, URL normalization, app/website/default matching, enabled-rule checks, config-list mutation/reordering, enabled-list display, and save validation; macOS uses `VoiceInkPowerModeValidationError` directly while keeping SwiftUI alerts, storage timing, notifications, shortcuts, and OS-trigger discovery in the shell.
