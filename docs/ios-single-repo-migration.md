...
- macOS `PowerModeManager` and `PowerModeValidator` delegate URL normalization, app/website/default matching, enabled-rule checks, and save validation to `VoiceInkPowerModePolicy`; macOS uses `VoiceInkPowerModeValidationError` directly while keeping the stored `PowerModeConfig` shape as the shell adapter.
