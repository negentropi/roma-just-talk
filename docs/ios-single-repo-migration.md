...
- `iOS/VoiceInk-ios/LocalModelManager.swift` validates model-download HTTP responses through `VoiceInkWhisperModelDownloadResponsePolicy`; iOS onboarding and local-model management render downloaded/download-progress state, download prompts, and operation failure alerts through `VoiceInkWhisperModelDownloadState`/`VoiceInkWhisperModelDownloadProgress`/`VoiceInkWhisperModelOperationAlertPresentation`, while the iOS shell still owns URLSession task/progress state and file-operation execution.
...
22. iOS local model operation failure alerts route through `VoiceInkWhisperModelOperationAlertPresentation`.
