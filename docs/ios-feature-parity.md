...
| R03 | Dictated text returns to the originating field | [`TranscriptionPipeline`](../VoiceInk/Transcription/Engine/TranscriptionPipeline.swift), [`CursorPaster`](../VoiceInk/Paste/CursorPaster.swift) | `implemented, verification pending`: [`KeyboardDictationExchange`](../iOS/Shared/KeyboardDictationExchange.swift) binds one App Group result to the originating document; the app publishes success/failure and [`KeyboardViewController`](../iOS/VoiceInkKeyboard/KeyboardViewController.swift) consumes it once through `textDocumentProxy.insertText` | shared/iOS | Store behavior is covered by iOS unit tests; exact-HEAD CI and host-app return/insertion simulator or device proof remain required. |
