...
- dashboard metrics summary, note-list summary presentation, and derived productivity math for time saved, words per minute, and keystrokes saved; platform shells still own metric queries, caching, and rendering
...
- power-mode stored config Codable shape, legacy key decoding, UserDefaults config and active-config-id persistence, trigger-to-rule adaptation, config-list lookup/mutation/reordering, enabled-list selection, app/website/default matching, website URL normalization, save validation, auto-send key stored value/display/enablement state, active-session snapshot Codable shape with cleanup legacy decoding, and active-session UserDefaults persistence; macOS still owns OS app/website detection, SwiftUI editing/alerts, active-session application/restoration, shortcuts, auto-send key execution, and config/session persistence timing
...
- macOS `PowerModeSessionManager` stores and loads `VoiceInkPowerModeSession` plus `VoiceInkPowerModeApplicationState` through `VoiceInkPowerModeSessionPreference` while keeping provider/model application, cleanup restoration, notifications, recovery timing, and error logging in the macOS shell.
...
- macOS history/import row summaries, macOS audio-file row action text, macOS transcript original/enhanced tabs, and iOS note detail display/retry labels use `VoiceInkTranscriptPresentation`/`VoiceInkTranscriptTextVariant` for the enhanced-text-first transcript display rule, shared empty-content fallback copy, and shared transcript-detail presentation copy.
...
- macOS transcription services, Power Mode, rolling preload, and iOS `AppSettings` read/write/clear selected transcription language through `VoiceInkTranscriptionLanguagePreference`; macOS fallback language selection and Native Apple source-compatible fallback live in shared core while platform shells still own settings UI and language-change notifications.
...
- iOS retry and background recording transcription now apply stored-audio retranscription success, missing-audio, and failure state through the shared `VoiceInkMutableTranscriptionRecord` + `VoiceInkStoredAudioRecord` default, and `RecordingManager` creates pending rows through `VoiceInkRecordingTranscriptionDraft`, leaving the iOS shell responsible only for settings, provider adapters, SwiftData saves, and recording cleanup.
...
- iOS `Transcription` records conform to both `VoiceInkSessionMetricSource` and `VoiceInkDashboardMetricRecord`, inheriting dashboard word/audio values from the shared session metric policy; `iOS/VoiceInk-ios/NotesListView.swift` builds its note-list word/audio and fastest-model summary through `VoiceInkNoteListSummaryPresentation`, so the iOS shell consumes the same dashboard metric and model performance interfaces as macOS.
...
21. iOS recording/retry mode selection, recording sheet controls, settings mode summaries, and mode-configuration form copy route presentation through `VoiceInkModeSelectionPresentation`, `VoiceInkRecordingSheetPresentation`, `VoiceInkModeSummaryPresentation`, and `VoiceInkModeFormPresentation`.
22. iOS local model row status, operation confirmations, and failure alerts route through `VoiceInkWhisperModelDownloadRowPresentation`, `VoiceInkWhisperModelOperationConfirmationPresentation`, and `VoiceInkWhisperModelOperationAlertPresentation`.
23. iOS API-key form copy and list status route through `VoiceInkProviderAPIKeyFormPresentation` and `VoiceInkProviderAPIKeyListRowPresentation`.
24. macOS and iOS local Whisper wrappers both consume shared runtime, VAD resource, and PCM16 WAV sample policies.
25. macOS AI-enhancement diagnostics/context toggles, iOS audio-session timeout presentation, and first-run onboarding storage checks stay routed through shared `VoiceInkCore` preference Modules.
26. macOS and iOS transcription-cleanup settings labels route through `VoiceInkTranscriptionCleanupPresentation`, and filler-word insertion stays routed through `VoiceInkFillerWords.insertPlan`, with duplicate copy out of platform views.
27. iOS dictionary settings plus macOS dictionary form/chrome, quick-add, word-replacement guidance, edit-sheet, and list/row copy route through `VoiceInkDictionarySettingsPresentation`/`VoiceInkDictionaryQuickAddPresentation`/`VoiceInkWordReplacementInfoPresentation`/`VoiceInkWordReplacementEditPresentation`/`VoiceInkVocabularyListPresentation`/`VoiceInkWordReplacementListPresentation`, with duplicate labels/placeholders/help text out of platform views.
