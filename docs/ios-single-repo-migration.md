...
- macOS cloud and streaming transcription adapters fetch normalized custom-vocabulary terms through `CustomVocabularyService`, so provider adapters no longer repeat SwiftData vocabulary fetch and normalization code; Deepgram still supplies its provider-specific term limit at the call site.
