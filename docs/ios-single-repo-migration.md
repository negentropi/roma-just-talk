...
- provider API-key draft verification candidate, save-after-verify policy, verification progress vocabulary, and verification feedback presentation for provider-key entry; macOS AI-enhancement provider-key entry shares draft enablement and environment-reference resolution while preserving shell key storage
...
23. iOS API-key list status routes through `VoiceInkProviderAPIKeyListRowPresentation`.
