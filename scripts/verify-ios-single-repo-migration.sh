...
require_pattern \
  "macOS transcription auto cleanup uses shared stored-audio reporting delete helper" \
  'deleteExistingAudioFileReportingFailure' \
  VoiceInk/Services/TranscriptionAutoCleanupService.swift
...
require_patterns \
  ...
  'testAudioInputDiagnosticsPreserveMacOSLogCopy' \
  'testMacOSAudioInputSettingsPresentationPreservesCopyAndIcons' \
...
require_patterns \
  "macOS audio device manager uses shared diagnostics and log category identity" \
  VoiceInk/Services/AudioDeviceManager.swift \
  'VoiceInkAudioInputDiagnostics\.' \
...
require_patterns \
  "macOS recorder uses shared audio-input notification presentation" \
  VoiceInk/Recorder.swift \
  'VoiceInkMacOSAudioInputSettingsPresentation\.macOS\.switchedDeviceNotificationTitle' \
  'VoiceInkMacOSAudioInputSettingsPresentation\.macOS\.usingDeviceNotificationTitle'
...
reject_pattern \
  "macOS audio device manager avoids shell-owned diagnostics and log category" \
  'category: "AudioDeviceManager"|"Failed to get system default device:|"🎙️ Using System Default mode"|"🎙️ Saved device UID|"🎙️ Current device unavailable, selecting new device\.\.\."|"No input devices available!"|"Unknown Device"|"🎙️ Auto-selecting new device:|"Error getting audio devices:|"🎙️ Currently selected device is no longer available"|"Error checking input capability for device|"Error getting stream configuration for device|"Attempted to select unavailable device:|"🎙️ Selected prioritized device:|"🎙️ No built-in input found, auto-selecting safe non-Bluetooth device:|"🎙️ No safe automatic input found; refusing to auto-select|"Failed to add device change listener:|"🎙️ Device list change detected"|"🎙️ Recording device|"🎙️ No priority devices available, using fallback"|"No audio input devices available!"|"Failed to get device property|"Failed to get transport type for device' \
  VoiceInk/Services/AudioDeviceManager.swift
...
reject_pattern \
  "macOS recorder avoids shell-owned audio input notification copy" \
  '"(Switched to:|Using:)' \
  VoiceInk/Recorder.swift
...
require_pattern \
  ...
  'macOS audio-input storage keys, selected-device UID persistence, last-used microphone notification suppression, priority-device JSON storage, audio-device change/switch notification contract, settings labels, status copy, empty states, action icons, priority display text, microphone notification title copy, manager diagnostic copy/category identity, current-device and mode-change selection planning, recording-device switch planning, and safe automatic input selection route through `VoiceInkAudioInputPreference`/`VoiceInkMacOSAudioDeviceChangeRequest`/`VoiceInkMacOSAudioInputSettingsPresentation`/`VoiceInkAudioInputDiagnostics`/`VoiceInkMacOSLogCategory`/`VoiceInkAudioInputSelectionPolicy`/`VoiceInkAudioInputAutomaticSelectionPolicy`' \
  docs/ios-single-repo-migration.md
...
require_multiline_pattern \
  "shared provider API-key stored-key mutator returns persistence actions" \
  'fileprivate mutating func applyStoredAPIKey\([[:space:]]+_ key: String,[[:space:]]+for provider: VoiceInkProviderKind[[:space:]]+\) -> \[VoiceInkProviderAPIKeyStatePersistenceAction\]' \
  VoiceInkCore/Sources/VoiceInkCore/ProviderAPIKeyState.swift

require_multiline_pattern \
  "shared provider API-key verification mutator returns persistence actions" \
  'fileprivate mutating func applyVerification\([[:space:]]+_ verified: Bool,[[:space:]]+for provider: VoiceInkProviderKind[[:space:]]+\) -> \[VoiceInkProviderAPIKeyStatePersistenceAction\]' \
  VoiceInkCore/Sources/VoiceInkCore/ProviderAPIKeyState.swift
...
reject_pattern \
  "macOS API-key manager avoids shallow AI enhancement verification wrapper" \
...
require_patterns \
  ...
  'isConfigureAPIKeysActionVisible' \
  'selectedConfigureAPIKeysAction'
...
require_pattern \
  ...
  'VoiceInkSettingsBackupCategory|VoiceInkSettingsBackupImportPolicy\.(currentVersion|versionReview|importSelectionReview|importSuccessPlan|needsAPIKeyReminder)|VoiceInkSettingsBackupPresentation\.macOS|backupPresentation\.(defaultFileName|exportPanelTitle|exportSuccessMessage|importPanelTitle|versionMismatchMessage|importSuccessInformativeText)' \
  VoiceInk/Services/ImportExportService.swift

reject_pattern \
  "macOS import/export avoids shell-owned backup import review flow" \
  'if let version = Bundle\.main\.object|currentSettingsVersion = "0\.0\.0"|backup\.version != currentSettingsVersion|guard let selectedCategories|guard !selectedCategories\.isEmpty|let needsAPIKeyReminder|needsAPIKeyReminder && response == \.alertSecondButtonReturn' \
  VoiceInk/Services/ImportExportService.swift
...
require_patterns \
  ...
  iOS/VoiceInk-ios/SettingsView.swift \
  'VoiceInkFillerWordDraftState' \
  'fillerWordSubmission\(fillerWordDraftState\)' \
  'applyFillerWordSubmission' \
  'settings\.fillerWords = \$0'
...
require_pattern \
  "macOS audio cleanup settings use shared toggle policy" \
  'VoiceInkMacOSCleanupSettingsPolicy\.(shouldShowAudioCleanupSection|transcriptCleanupChangePlan|audioCleanupChangePlan)' \
  VoiceInk/Views/Settings/AudioCleanupSettingsView.swift
...
require_patterns \
  ...
  VoiceInkCore/Sources/VoiceInkCore/DictionaryPolicy.swift \
  'struct VoiceInkDictionarySettingsSnapshot' \
  'var sortedCustomVocabularyTerms' \
  'var sortedWordReplacements' \
  'fillerWordSubmission' \
  'customVocabularySubmission' \
  'wordReplacementSubmission' \
  'applyFillerWordSubmission' \
  'applyCustomVocabularySubmission' \
  'applyWordReplacementSubmission' \
  'removingFillerWords' \
  'removingCustomVocabularyTerms' \
  'removingWordReplacements'

require_patterns \
  "shared dictionary settings snapshot checks run in VoiceInkCore" \
  VoiceInkCore/Tests/VoiceInkCoreTests/VoiceInkCoreCheckRunner.swift \
  'testDictionarySettingsSnapshotBuildsDisplayedRows' \
  'testDictionarySettingsSnapshotDeletesDisplayedRowsThroughOriginalStorageOrder' \
  'testDictionarySettingsSnapshotSubmitsAndAppliesDrafts' \
  'testDictionarySettingsSnapshotSkipsRuntimeSettersWhenDraftsDoNotChangeStorage'
...
require_patterns \
  ...
  iOS/VoiceInk-ios/SettingsView.swift \
  'VoiceInkWordReplacementDraftState' \
  'dictionarySnapshot' \
...
require_pattern \
  ...
  'VoiceInkAIEnhancementAPIKeyVerificationApplicationPlan|verificationApplicationPlan\(|applyRuntimeState|providerKeyStorageNameToSave' \
  VoiceInkCore/Sources/VoiceInkCore/AIProviderCatalog.swift

reject_pattern \
  "shared macOS AI API-key verification application plan avoids public raw result and persistence fields" \
  'public let (isValid|runtimeAPIKey|keyToSave|providerKeyStorageNameToSave|errorMessage)' \
  VoiceInkCore/Sources/VoiceInkCore/AIProviderCatalog.swift

reject_pattern \
  "shared macOS AI API-key verification subplans stay internal implementation details" \
  'public struct +VoiceInkAIEnhancementAPIKeyVerification(PersistencePlan|ServiceStatePlan)' \
  VoiceInkCore/Sources/VoiceInkCore/AIProviderCatalog.swift
...
require_patterns \
  "shared macOS AI API-key verification service-state application lives in VoiceInkCore" \
  VoiceInkCore/Sources/VoiceInkCore/AIProviderCatalog.swift \
...
require_pattern \
  "core checks execute shared macOS AI API-key verification persistence application test" \
  'AIProviderCatalogTests\.testMacOSAIEnhancementAPIKeyVerificationPlanAppliesSuccessPersistence' \
  VoiceInkCore/Tests/VoiceInkCoreTests/VoiceInkCoreCheckRunner.swift

require_pattern \
  "core checks execute shared macOS AI API-key verification service-state application test" \
  'AIProviderCatalogTests\.testMacOSAIEnhancementAPIKeyVerificationServiceStatePlanAppliesRuntimeState' \
  VoiceInkCore/Tests/VoiceInkCoreTests/VoiceInkCoreCheckRunner.swift

require_pattern \
  "core checks execute shared macOS AI API-key verification application runtime test" \
  'AIProviderCatalogTests\.testMacOSAIEnhancementAPIKeyVerificationPlanAppliesRuntimeState' \
  VoiceInkCore/Tests/VoiceInkCoreTests/VoiceInkCoreCheckRunner.swift

require_pattern \
  "shared macOS AI API-key clear plan lives in VoiceInkCore" \
  'VoiceInkAIEnhancementAPIKeyClearPlan|credentialStateAfterClear|providerKeyStorageNameToDelete|clearing\(provider:|applyRuntimeState' \
  VoiceInkCore/Sources/VoiceInkCore/AIProviderCatalog.swift

reject_pattern \
  "shared macOS AI API-key clear plan avoids public raw persistence and state fields" \
  'public let (providerKeyStorageNameToDelete|credentialStateAfterClear)' \
  VoiceInkCore/Sources/VoiceInkCore/AIProviderCatalog.swift

reject_pattern \
  "shared macOS AI API-key clear subplans stay internal implementation details" \
  'public struct +VoiceInkAIEnhancementAPIKeyClear(PersistencePlan|ServiceStatePlan)' \
  VoiceInkCore/Sources/VoiceInkCore/AIProviderCatalog.swift
...
require_patterns \
  "shared macOS AI API-key clear service-state application lives in VoiceInkCore" \
  VoiceInkCore/Sources/VoiceInkCore/AIProviderCatalog.swift \
...
require_pattern \
  "core checks execute shared macOS AI API-key clear service-state application test" \
  'AIProviderCatalogTests\.testMacOSAIEnhancementAPIKeyClearServiceStatePlanAppliesRuntimeState' \
  VoiceInkCore/Tests/VoiceInkCoreTests/VoiceInkCoreCheckRunner.swift

require_pattern \
  "core checks execute shared macOS AI API-key clear runtime application test" \
  'AIProviderCatalogTests\.testMacOSAIEnhancementAPIKeyClearPlanAppliesRuntimeState' \
  VoiceInkCore/Tests/VoiceInkCoreTests/VoiceInkCoreCheckRunner.swift
...
require_patterns \
  ...
  VoiceInk/Services/AIEnhancement/AIService.swift \
  'VoiceInkAIEnhancementAPIKeyClearPlan\.clearing' \
  'applyTextEnhancementAPIKeyClearPlan' \
  'plan\.applyRuntimeState' \
  'deleteKey:'

reject_pattern \
  "macOS API-key manager avoids shallow AI clear persistence wrapper" \
  'applyAIEnhancementAPIKeyClearPlan|VoiceInkAIEnhancementAPIKeyClearPlan|applyClearPersistence' \
  VoiceInk/Services/APIKeyManager.swift

reject_pattern \
  "macOS AI service avoids split clear persistence and state application" \
  'APIKeyManager\.shared\.applyAIEnhancementAPIKeyClearPlan' \
  VoiceInk/Services/AIEnhancement/AIService.swift
...
require_pattern \
  ...
  'verification service-state application planning/execution.*VoiceInkAIEnhancementAPIKeyVerificationServiceStatePlan' \
  docs/ios-single-repo-migration.md

require_pattern \
  "migration checklist tracks shared macOS AI API-key clear plan" \
  'clear-key planning/application.*VoiceInkAIEnhancementAPIKeyClearPlan' \
  docs/ios-single-repo-migration.md
...
require_pattern \
  ...
  'clear-key service-state application planning/execution.*VoiceInkAIEnhancementAPIKeyClearServiceStatePlan' \
  docs/ios-single-repo-migration.md

require_patterns \
  "macOS AI service persists verified keys through shared verification plan" \
  VoiceInk/Services/AIEnhancement/AIService.swift \
  'plan\.applyRuntimeState' \
  'saveKey: \{ APIKeyManager\.shared\.saveAPIKey\(\$0, forProvider: \$1\) \}'

reject_pattern \
  "macOS AI service avoids split verified-key persistence and state application" \
  'APIKeyManager\.shared\.applyAIEnhancementVerificationPlan' \
  VoiceInk/Services/AIEnhancement/AIService.swift

reject_pattern \
  "macOS API-key manager avoids shallow AI verification persistence wrapper" \
  'applyAIEnhancementVerificationPlan|VoiceInkAIEnhancementAPIKeyVerificationApplicationPlan' \
  VoiceInk/Services/APIKeyManager.swift

reject_pattern \
  "macOS API-key manager avoids shell-owned AI verification persistence field reads" \
  'plan\.runtimeAPIKey|plan\.providerKeyStorageNameToSave|plan\.keyToSave|successPersistencePlan|persistencePlan\.providerKeyStorageNameToSave|saveAPIKey\(keyToSave, forProvider: providerKeyStorageNameToSave\)' \
  VoiceInk/Services/APIKeyManager.swift
...
require_pattern \
  "migration checklist tracks shared macOS AI verification application runtime plan" \
  'verification-result application planning/execution.*VoiceInkAIEnhancementAPIKeyVerificationApplicationPlan' \
  docs/ios-single-repo-migration.md
...
reject_pattern \
  "macOS AI service avoids shell-owned API-key verification service-state field reads" \
  'serviceStatePlan\.(apiKeyToApply|isAPIKeyValid|shouldPostProviderKeyChanged|completionResult)|serviceStateApplicationPlan\.(apiKeyToApply|isAPIKeyValid|shouldPostProviderKeyChanged|completionResult)' \
  VoiceInk/Services/AIEnhancement/AIService.swift
...
reject_pattern \
  ...
  'plan\.credentialStateAfterClear|plan\.providerKeyStorageNameToDelete|serviceStatePlan\.(credentialStateAfterClear|shouldPostProviderKeyChanged)|serviceStateApplicationPlan\.(credentialStateAfterClear|shouldPostProviderKeyChanged)' \
  VoiceInk/Services/AIEnhancement/AIService.swift
...
require_pattern \
  "macOS custom sound manager applies shared copy runtime plan" \
  'copyPlan\.applyRuntimeState|removeExistingDestination|copyToDestination' \
  VoiceInk/CustomSoundManager.swift
...
reject_pattern \
  "macOS custom sound manager avoids raw copy action branching" \
  'copyPlan\.action|case \.(useExistingDestination|replaceExistingDestinationAndCopy|copy)' \
  VoiceInk/CustomSoundManager.swift

reject_pattern \
  "recording feedback tests exercise copy runtime behavior instead of raw actions" \
  'plan\.action|useExistingDestination|replaceExistingDestinationAndCopy' \
  VoiceInkCore/Tests/VoiceInkCoreTests/RecordingFeedbackPreferenceTests.swift
...
require_pattern \
  "macOS audio player applies shared timer tick runtime plan" \
  'plan\.applyRuntimeState' \
  VoiceInk/Views/AudioPlayerView.swift
...
require_pattern \
  "iOS audio player applies shared timer tick runtime plan" \
  'plan\.applyRuntimeState' \
  iOS/VoiceInk-ios/AudioPlayer.swift
...
reject_pattern \
  "platform audio players avoid shell-owned timer tick side-effect unpacking" \
  'plan\.(shouldStopTimer|playerSeekTime)' \
  VoiceInk/Views/AudioPlayerView.swift \
  iOS/VoiceInk-ios/AudioPlayer.swift
...
reject_pattern \
  "iOS mode model selection avoids direct presentation case switching" \
  'switch +provider\.modelSelectionPresentation|case \.(fixedModel|selectableModels)' \
  iOS/VoiceInk-ios/ModeConfigurationView.swift
...
reject_pattern \
  "iOS audio-session manager avoids shell-owned active-session branching" \
  '(if|guard) +lifecycleState\.isSessionActive' \
  iOS/VoiceInk-ios/AudioSessionManager.swift
...
require_pattern \
  "macOS language selection persists local Whisper prompts through shared prompt policy" \
  'VoiceInkTranscriptionPromptPreference\.saveLocalWhisperPromptForSelectedLanguage\(\)' \
  VoiceInk/Views/AI\ Models/LanguageSelectionView.swift

require_pattern \
  "macOS cloud model card language repair persists local Whisper prompts through shared prompt policy" \
  'VoiceInkTranscriptionPromptPreference\.saveLocalWhisperPromptForSelectedLanguage\(\)' \
  VoiceInk/Views/AI\ Models/CloudModelCardView.swift

require_pattern \
  "macOS transcription model manager language repair persists local Whisper prompts through shared prompt policy" \
  'VoiceInkTranscriptionPromptPreference\.saveLocalWhisperPromptForSelectedLanguage\(\)' \
  VoiceInk/Transcription/Engine/TranscriptionModelManager.swift

require_pattern \
  "macOS Power Mode language application persists local Whisper prompts through shared prompt policy" \
  'VoiceInkTranscriptionPromptPreference\.saveLocalWhisperPromptForSelectedLanguage\(\)' \
  VoiceInk/PowerMode/PowerModeSessionManager.swift

require_patterns \
  "macOS model settings reads and saves local Whisper prompts through shared catalog" \
  VoiceInk/Views/ModelSettingsView.swift \
  'VoiceInkLocalWhisperPromptCatalog\.prompt' \
  'VoiceInkLocalWhisperPromptCatalog\.saveCustomPrompt' \
  'VoiceInkLocalWhisperPromptCatalog\.storedCustomPrompts'
...
require_patterns \
  ...
  VoiceInkCore/Tests/VoiceInkCoreTests/VoiceInkCoreCheckRunner.swift \
  'UserDefaultsPreferencesTests\.testTranscriptionPromptPreferenceUsesFallbackOnlyWhenLocalWhisperPromptIsMissing' \
  'UserDefaultsPreferencesTests\.testTranscriptionPromptPreferenceUsesSelectedLanguagePromptFallback' \
  'UserDefaultsPreferencesTests\.testTranscriptionPromptPreferenceSavesSelectedLanguagePrompt' \
  'UserDefaultsPreferencesTests\.testTranscriptionPromptPreferenceSavesCustomPromptForSelectedLanguage'
...
reject_file VoiceInk/Transcription/Whisper/WhisperPrompt.swift

reject_pattern \
  "macOS local Whisper prompt settings avoid obsolete prompt object wrapper" \
  '\bWhisperPrompt\b|whisperPrompt\.' \
  VoiceInk/Views/AI\ Models/ModelManagementView.swift \
...
require_pattern \
  "shared Power Mode transcription model-change plan lives in VoiceInkCore" \
  'VoiceInkPowerModeTranscriptionModelChangePlan|modelChangePlan|selectionToApply' \
  VoiceInkCore/Sources/VoiceInkCore/PowerModePolicy.swift
...
require_pattern \
  "macOS Power Mode config form delegates transcription model-change language repair planning" \
...
require_pattern \
  "shared Power Mode preference application runtime lives in VoiceInkCore" \
  'VoiceInkPowerModePreferenceApplication|applyRuntimeState|powerModePreference(Application|Restore)' \
  VoiceInkCore/Sources/VoiceInkCore/PowerModePolicy.swift

reject_pattern \
  "shared Power Mode preference application raw payload stays hidden" \
  'public enum +VoiceInkPowerModePromptSelectionApplication|public let +(isEnhancementEnabled|useScreenCaptureContext|promptSelection|selectedAIProvider|selectedAIModel|cleanupRestore)|public func applyPromptSelection' \
  VoiceInkCore/Sources/VoiceInkCore/PowerModePolicy.swift
...
require_pattern \
  "macOS Power Mode session manager applies shared preference runtime policy" \
  'application\.applyRuntimeState|setSelectedPromptId' \
  VoiceInk/PowerMode/PowerModeSessionManager.swift

reject_pattern \
  "macOS Power Mode session manager avoids raw preference application fields" \
  'application\.(isEnhancementEnabled|useScreenCaptureContext|selectedAIProvider|selectedAIModel|cleanupRestore|applyPromptSelection)' \
  VoiceInk/PowerMode/PowerModeSessionManager.swift
...
require_pattern \
  ...
  'VoiceInkDashboardPromotionPresentation|VoiceInkLicenseState|VoiceInkDashboardPromotionCardPresentation|badgeDisplayText|affiliateDismissedKey = "VoiceInkAffiliatePromotionDismissed"|socialShareURLString = "https://tryvoiceink\.com/social-share"|affiliateURLString = "https://tryvoiceink\.com/affiliate"|dismissSystemImageName = "xmark\.circle\.fill"|cards\(' \
  VoiceInkCore/Sources/VoiceInkCore/DashboardMetrics.swift
...
reject_multiline_pattern \
  ...
  'private[[:space:]]+func[[:space:]]+[[:alnum:]_]+[[:space:]]*\([[:space:]]*\)[[:space:]]*(->[[:space:]]*(Void|\(\))[[:space:]]*)?\{([[:space:]]|//[^\n]*\n|/\*([^*]|\*+[^*/])*\*/|guard[^{}]*else[[:space:]]*\{[[:space:]]*return[[:space:]]*\})*(return[[:space:]]+)?VoiceInkKeyboardURLOpener[[:space:]]*\.[[:space:]]*openMainApp[[:space:]]*\([^)]*url[[:space:]]*:[[:space:]]*VoiceInkAppDeepLink[[:space:]]*\.[[:space:]]*record[[:space:]]*\.[[:space:]]*url' \
  iOS/VoiceInkKeyboard/KeyboardViewController.swift
...
reject_pattern \
  ...
  'switch tapPlan\.action|tapPlan\.shouldRefreshButtonStateAfterAction|case \.requestStopRecording|case \.openMainAppForRecording' \
  iOS/VoiceInkKeyboard/KeyboardViewController.swift

require_patterns \
  "VoiceInkCore owns iOS keyboard stop-request policy" \
  VoiceInkCore/Sources/VoiceInkCore/RecordingStatePolicy.swift \
  'VoiceInkKeyboardStopRecordingRequestPolicy' \
...
reject_pattern \
  "iOS app shell avoids shell-owned keyboard stop-request action switches" \
  'switch VoiceInkKeyboardStopRecordingRequestPolicy|case \.handleStopRequest|case \.ignore' \
  iOS/VoiceInk-ios/RecordingManager.swift \
  iOS/VoiceInk-ios/NotesListView.swift
