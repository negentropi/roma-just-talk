...
- vocabulary, word-replacement, backup dictionary insert, vocabulary backup record shape, word-replacement backup import planning, word-replacement edit planning, dictionary draft submit rules, dictionary alert titles, and dictionary persistence failure copy; platform shells still own macOS SwiftData dictionary storage and persistence execution
...
27. macOS Native Apple language-display fallback stays routed through `VoiceInkLanguageCatalog.nativeAppleDisplayName`.
28. iOS note-list dashboard and fastest-model summaries stay routed through `VoiceInkNoteListSummaryPresentation`.
29. macOS and iOS duration-dependent UI stays routed through `VoiceInkDurationPresentation.shouldShowPositiveDuration`.
30. A real Xcode toolchain is selected and both app targets build.
